...
#retrive users
$userStmt = $db->prepare("SELECT * FROM users WHERE id=$userId");
$userStmt->execute();
...
if(isset($_POST['userUpdateBtn'])) {
   ...
   $stmt = $db->prepare("UPDATE users SET name=:name, email=:email, role=:role WHERE id=$userId");
        ...
        echo "<script>sweeetAlert(' upadated a user', 'users')</script>";
         }
}
?>
<!DOCTYPE html>
                ...
                <div class="card-body">
                    <form action="" method="post">
                        ...
                        <div class="mb-2">
                            ...
                            <input type="text" name="password" class="form-control" id="checkInput" value="<?php echo $user->password ?>">
                        ...
                        <button type="submit" name="userUpdateBtn" class="btn btn-info btn-sm">Update</button>
                    </form>
                </div>
            </div>
    </div>
